...
class MultiStageDecoderLayer(nn.Module):
    def __init__(self, d_model, dim_feedforward=2048, dropout=0.1,
        ...
        # tgt自注意
        # self.tgt_attn = nn.MultiheadAttention(embed_dim=256, num_heads=8, dropout=0.1)
        # 视觉注意
        # self.fv_attn = nn.MultiheadAttention(embed_dim=256, num_heads=8, dropout=0.1)
        ...
        self.norm = _get_clones(nn.LayerNorm(d_model), 4)
        self.dropout = _get_clones(nn.Dropout(dropout), 4)
    ...
    def forward(self, vis_query, vis_query_pos, text_query_pos,
        ...
        # 使用了语言验证
        if self.img_feat_chunk_num > 1:
            word_feat_srcs = word_feat.chunk(self.img_feat_chunk_num, dim=-1)
            word_feat_k = word_feat_srcs[1]
            word_feat_v = word_feat_srcs[0]
        else:
            word_feat_k = word_feat_v = word_feat


        # tgt自注意
        # vis_query = self.tgt_attn(query=self.with_pos_embed(vis_query, vis_query_pos),
        #                            key=self.with_pos_embed(vis_query, vis_query_pos),
        #                            value=vis_query)[0]
        # vis_query = self.norm[3](self.dropout[3](vis_query))


        # 先视觉后文本
        # vis_query = self.fv_attn(query=self.with_pos_embed(vis_query, vis_query_pos),
        #                            key=self.with_pos_embed(img_feat_v, img_pos),
        #                            value=img_feat_v)[0]
        # vis_query = self.norm[3](self.dropout[3](vis_query))
        ...
        #                            key=self.with_pos_embed(word_feat_k, word_pos),
        #                            value=word_feat_v, key_padding_mask=word_key_padding_mask)[0]
        # 不加query残差的
        # text_query = self.norm[0](self.dropout[0](text_info))

        # 加上残差的
        # text_query = self.norm[0](self.dropout[0](text_info) + vis_query)


        # 在文本注意之前加的
        # vis_query = self.word_attn(query=self.with_pos_embed(vis_query, vis_query_pos),
        #                            key=self.with_pos_embed(word_feat_k, word_pos),
        #                            value=word_feat_v, key_padding_mask=word_key_padding_mask)[0]
        # vis_query = self.norm[3](self.dropout[3](vis_query))
        word_feat = word_feat_v
        ...
        # 在语言之后加
        vis_query = self.word_attn(query=self.with_pos_embed(text_query, vis_query_pos),
                                   key=self.with_pos_embed(word_feat_k, word_pos),
                                   value=word_feat_v, key_padding_mask=word_key_padding_mask)[0]
        text_query = self.norm[3](self.dropout[3](vis_query) + text_query)
        # word_feat = word_feat_v


        # 先文本后视觉
        # text_query = self.fv_attn(query=self.with_pos_embed(text_query, text_query_pos),
        #                           key=self.with_pos_embed(img_feat_v, img_pos),
        #                           value=img_feat_v)[0]
        # text_query = self.norm[3](self.dropout[3](text_query))
        ...
        # 加text_info的残差
        # vis_query = self.norm[1](text_info + self.dropout[1](vis_info))

        # 都加上
        vis_query = self.norm[1](text_info + vis_query + self.dropout[1](vis_info))
...
# 返回语言得分
class DiscriminativeFeatEncLayer_text(nn.Module):
    def __init__(self, d_model, img_query_with_pos=True):
        ...
        self.img2img_attn = nn.MultiheadAttention(embed_dim=256, num_heads=8, dropout=0.1)
    ...
    def forward(self, img_feat, img_key_padding_mask, img_pos,
        ...
        text_info = self.img2text_attn(
            query=self.with_pos_embed(word_feat, word_pos), key=img_query,
            value=img_feat, key_padding_mask=img_key_padding_mask)[0]

        text_embed = self.text_proj(text_info)
        word_embed = self.img_proj(word_feat)
        # print(img_embed.shape, 'img_embed')
        verify_score = (F.normalize(word_embed, p=2, dim=-1) *
                        F.normalize(text_embed, p=2, dim=-1)).sum(dim=-1, keepdim=True)
        # print(verify_score.shape, 'verify_score1')
        verify_score = self.tf_scale * \
                       torch.exp( - (1 - verify_score).pow(self.tf_pow) \
                        / (2 * self.tf_sigma**2))
        # print(verify_score.shape, 'verify_score2')
        # torch.Size([400, 8, 256])   img_embed
        # torch.Size([400, 8, 1]) verify_score1
        # torch.Size([400, 8, 1]) verify_score2
        ...
        q = k = word_feat + text_cond_info
        text_cond_img_ctx = self.img2img_attn(
            query=q, key=k, value=word_feat, key_padding_mask=word_key_padding_mask)[0]
        # discriminative feature
        fuse_word_feat = (self.norm_img(word_feat) +
                         self.norm_text_cond_img(text_cond_img_ctx)) * verify_score

        return torch.cat([orig_word_feat, fuse_word_feat], dim=-1)
...
class DecoderWithExtraEncoder(nn.Module):
    def __init__(self, num_queries, query_dim,
        ...
        # 返回语言得分的
        text_encoder = DiscriminativeFeatEncLayer_text(256)
        self.text_encoder_layers = _get_clones(text_encoder, num_extra_layers)
    ...
    def forward(self, img_feat, img_key_padding_mask=None, pos=None,
        ...
        for layer in self.text_encoder_layers:
            word_feat = layer(o_img_feat, img_key_padding_mask, pos,
                             word_feat, word_key_padding_mask, None)
